...
public:
	...
	bool gameOver(Show s, Users& user);
	void get_grade(Users user, wchar_t*& wstr);
	void get_assuse(wchar_t*& wstr);
};
class AiChess : public Chess{
public:
	static bool useZhiBaMode;
	static bool useHellMode;
	static bool useOptimalMode; // true = 最优策略，false = 普通策略
	bool is_Ok(int x, int y){return x >= 0 && y >= 0 && x < 18 && y < 18;}
	...
	bool gameOver(Show s, Users& user);
	int EvaluatePoint(int, int, int);
	int AlphaBetaAI(vector<vector<int>>& board, int player, int depth, int alpha, int beta);
	int EvaluateBoard(vector<vector<int>>& board, int player);
	POINT FindBestMove_ZhiBa(vector<vector<int>>& board, int player);
};
